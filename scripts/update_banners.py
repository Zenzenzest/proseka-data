...
def main():
    ...
    all_en_banners_so_far = existing_en_banners.copy()  # Start with existing banners
    new_en_banners = []
    ...
    # Merge with existing EN banners
    all_en_banners = existing_en_banners + new_en_banners
    
    # Update JP banners with en_id from EN banners
    updated_jp_banners = update_jp_banners_with_en_ids(all_jp_banners, all_en_banners)
    ...
    with open('en_banners.json', 'w', encoding='utf-8') as f:
        json.dump(all_en_banners, f, ensure_ascii=False, indent=2)
