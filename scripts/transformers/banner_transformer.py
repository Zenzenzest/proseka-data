...
from .mappings import CHARACTERS, UNIT_PREMIUM, JP_NAME_MAPPINGS
import pytz
...
def convert_jp_time_to_en_rerun(jp_start_time: int, jp_end_time: int) -> tuple:
    ...
    if en_start_dt.month == 12:
        next_month = en_start_dt.replace(
            year=en_start_dt.year + 1, month=1, day=1)
    else:
    ...
    rerun_start_dt = jp_start_dt.replace(
        year=jp_start_dt.year + 1) - timedelta(days=5)
    ...
    rerun_end_dt = jp_end_dt.replace(
        year=jp_end_dt.year + 1) + timedelta(days=5)
...
def create_en_banner_from_jp(jp_banner: Dict, en_banners: List[Dict], jp_banners: List[Dict]) -> Dict:
    jp_banner_copy = jp_banner.copy()
    if jp_banner_copy.get("banner_type") == "Collab":
        return False
    en_banner = {
        "id": get_next_id(en_banners),
    ...
    if jp_banner_copy.get("banner_type") == "Birthday":
        ...
        cards = jp_banner_copy.get("cards", [])
        previous_name = find_previous_birthday_banner_name(
            cards, en_banners)
        if previous_name:
    ...
    # special handling for Limited Event Rerun
        ...
        jp_start = jp_banner_copy.get("start", 0)
        jp_end = jp_banner_copy.get("end", 0)
        en_start, en_end, rerun_start, rerun_end = convert_jp_time_to_en_rerun(
            jp_start, jp_end)
        en_banner["start"] = en_start
        en_banner["end"] = en_end
...
def find_original_limited_event_name(cards: List[int], en_banners: List[Dict], jp_banners: List[Dict]) -> str:

    if not cards:
        return ""
    first_card_id = cards[0]
    for banner in en_banners:
...
def find_previous_birthday_banner_name(cards: List[int], en_banners: List[Dict]) -> str:
...
def update_en_banners_from_en_source(en_diff: List[Dict], en_banners: List[Dict], en_gachas_changes: List[Dict] = None) -> List[Dict]:
    cards_data = []
    if os.path.exists('cards.json'):
        with open('cards.json', 'r', encoding='utf-8') as f:
            cards_data = json.load(f)
    en_banner_lookup = {banner.get(
        "sekai_id"): banner for banner in en_banners}

    # Determine which gachas to process
    if en_gachas_changes:
        # Process only the changed gachas (incremental update)
        gachas_to_process = [
            gacha for gacha in en_gachas_changes if gacha.get("id", 0) > 570]
        print(f"Processing {len(gachas_to_process)} changed EN gachas")
    else:
        # Fallback to original behavior - process all gachas above 570
        gachas_to_process = [
            gacha for gacha in en_diff if gacha.get("id", 0) > 570]
    ...
    updated_banner_ids = []
    added_banner_ids = []
    result_banners = en_banners.copy()
    ...
    for gacha in gachas_to_process:
        ...
        gacha_details_card_ids = extract_rarity_4_card_ids(
            gacha.get("gachaDetails", []), cards_data, banner_type)
...
def group_birthday_banners(gachas: List[Dict]) -> List[Dict]:
    birthday_gachas = [g for g in gachas if g.get(
        "id", 0) >= 800 and is_birthday_banner(g.get("name", ""))]
    ...
    for jp_name, banners in grouped.items():
        if banners:
            ...
            # combined card IDs
            first_banner["gachaPickups"] = [
                {"cardId": card_id} for card_id in all_cards]
...
def determine_banner_type(name: str, cards: List[int], cards_data: List[Dict]) -> str:
    ...
    elif "セレクトリスト" in name or "Your Pick" in name:
        return "Your Pick"
    elif "メモリアルセレクト" in name or "Memorial Select" in name:
        return "Memorial Select"
    elif "ルームフェスティバル" in name or "Bloom Festival" in name:
        return "Bloom Festival"
    elif "メンバー確定" in name:
        return "4★ Guaranteed"
    elif "スペシャルプライズ" in name:
        return "Special Prize"
    elif "ワールドリンクイベントサポート" in name:
        return "World Link Support"
    ...
    if cards and len(cards) >= 3 and cards_data:
        ...
        if len(first_three_cards) == 3:
            card_types = [card.get("card_type", "")
                          for card in first_three_cards]

            # All must be same type
            if len(set(card_types)) == 1:
                first_type = card_types[2]
                if first_type == "limited":
                    return "Limited Event"
                elif first_type == "permanent":
                    return "Event"
...
def update_en_banners(en_diff: List[Dict], en_banners: List[Dict], jp_cards: List[Dict]) -> List[Dict]:


    lookup = {banner['id']: banner for banner in en_diff}
    existing_sekai_ids = {item['sekai_id'] for item in en_banners}
    highest_id = max([item.get('id', 0) for item in en_banners]) if en_banners else 0
    next_id = highest_id + 1
    for banner in en_banners:
        if banner["sekai_id"] in lookup:
            for key, value in lookup[banner["sekai_id"]].items():
                if key == "name":
                    banner["name"] = value
                elif key == "endAt":
                    banner["end"] = value
                    ...
                    banner["start"] = value
                elif key == "gachaDetails":
                    cards = get_gacha_details(
                        value.get("gachaDetails"), jp_cards)
                    banner["gachaDetails"] = cards

    # If new banner
    for banner in en_diff:
        if banner["id"] not in existing_sekai_ids:

            if "Happy Birthday" in banner["name"] or "Happy Anniversary" in banner["name"]:
                continue

            cards = []
            for card in banner["gachaPickups"]:
                card_id = card.get("cardId")
                cards.append(card_id)
            gachaDetails = get_gacha_details(
                banner.get("gachaDetails"), jp_cards)
            new_banner = {
                "id": next_id,
                "name": banner["name"],
                "sekai_id": banner["id"],
                "start": banner["startAt"],
                "end": banner["endAt"],
                "cards": cards,
                "banner_type": determine_banner_type(banner["name"], cards, jp_cards),
                "keywords": [],
                "characters": get_character_ids_from_cards(cards, jp_cards, CHARACTERS),
                "gachaDetails": gachaDetails
            }
            en_banners.append(new_banner)
            
    return en_banners


def get_gacha_details(cards: List[Dict], jp_cards: List[Dict]) -> List[int]:
    cd = []
    for card in cards:
        card_id = card.get("cardId")
        for c in jp_cards:
            if c.get("id") == card_id:
                if c.get("card_type") == "limited_collab":
                    cd.append(card_id)
                elif c.get("rarity") == 4:
                    cd.append(card_id)
    return cd
...
def convert_birthday_banner_names(name: str) -> str:
    match = re.match(r"\[(.*?)\](.*)", name)
    if not match:
        return name
    jp_name, rest = match.groups()
    romaji_name = JP_NAME_MAPPINGS.get(jp_name, jp_name)

    if "メモリーズ" in rest:
        rest = rest.replace("メモリーズ", " Memories")
        rest = rest.replace("ガチャ", " Gacha")
        return f"{romaji_name} {rest.title()}"
    ...
    if year_match:
        ...
        rest = re.sub(r"(\d{4})", f" {year}", rest, count=1)

    rest = rest.replace("ガチャ", " Gacha")

    return f"{romaji_name} {rest.title()}".strip()


def transform_diff(banners: List[Dict], mode: str, jp_banners: List[Dict], jp_cards: List[Dict], en_banners: List[Dict]) -> List[Dict]:
    transformed_banners = []
    index = 1
    bday_index = 0
    banners_length = len(banners)
    for banner in banners:
        name = banner.get("name")
        if "カラフルパスガチャ" in name:
            continue
        # Skip bday anniv reruns but not memories
        # this is useless because there won't be any individual bday/anniv reruns
        # and they're all compiled into a single memories banenr
        # i just realized that as I was writing this
        # oh well

        # if ("HAPPY BIRTHDAY" in name or "HAPPY ANNIVERSARY" in name) and "メモリーズ" not in name:
        #     bday_index += 1
        # if bday_index >= 1:
        #     for card in banner.get("gachaPickups"):
        #         cardId = card.get("cardId")
        #         transformed_banners[-1]["cards"].append(cardId)
        #     continue

        transformed_banner = transform_banner(
            banner, jp_banners, index, jp_cards, en_banners, mode, banners_length)
        transformed_banners.append(transformed_banner)
        index += 1

    return transformed_banners


def transform_banner(banner: Dict, jp_banners: List[Dict], index: int, jp_cards: List[Dict], en_banners: List[Dict], mode: str, banners_length: int) -> Dict:

    latest_jpid = jp_banners[-1]["id"]
    latest_enid = en_banners[-1]["id"]
    jpid = latest_jpid + index
    enid = latest_enid + index
    name = banner.get("name", "")
    sekai_id = banner.get("id", 0)
    start = banner.get("startAt", 0)
    end = banner.get("endAt", 0)
    cards = []
    keywords = []
    en_id = enid
    gachaDetails = []
    ...
    for card in banner.get("gachaDetails"):
        card_id = card.get("cardId")
        for c in jp_cards:
            if c.get("id") == card_id:
                if c.get("card_type") == "limited_collab":
                    gachaDetails.append(card_id)
                elif c.get("rarity") == 4:
                    gachaDetails.append(card_id)

    banner_type = determine_banner_type(name, cards, jp_cards)

    allowed_types = ["Limited Event", "Event", "Bloom Festival"]
    jp_banner = {
        "id": jpid,
        "name": name,
        "sekai_id": sekai_id,
    ...
    if banner_type in allowed_types:
        jp_banner["event_id"] = 0

    if banner_type == "Birthday":
        # find previous birthday banner name and increment year
        previous_name = convert_birthday_banner_names(name)
        if previous_name:
            en_banner["name"] = previous_name
            print(previous_name)
    ...
    # special handling for Limited Event Rerun
    elif banner_type == "Limited Event Rerun":
        ...
        en_start, en_end = convert_jp_time_to_en_normal(start, end)
        # Update time fields
        new_name = re.sub(r'\s*ガチャ$', ' Gacha', name)
        en_banner["name"] = new_name
        en_banner["start"] = en_start
        en_banner["end"] = en_end
    if mode == "jp":
        print(jp_banner)
